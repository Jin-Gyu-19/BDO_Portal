...
| `vuln` | 취약점 진단 | `extUrl(7010,7011)+'?embed=1'` → https면 `https://<접속호스트>:7011/?embed=1` | **외부 포트 직접 연결** (XBRL 과 같은 방식 — MS 로그인을 쓰지 않아 `auth` 게이트 없음). `7011`(HTTPS) → `7010`(앱). `embed=1` 은 회의실·경비청구와 같은 임베드 모드 규약. 창 1280×900 은 콘텐츠 폭을 재보지 못한 임시값. 2026-09-20 연결 |
