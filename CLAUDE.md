...
- **AI 활용사례 그룹(2026-09-19)**: 사용자 제공 아이콘 8종(`icons/aicase-*-20260919.webp`, 512px WebP로 변환해 `PORTAL_ASSETS`에 내장)으로 새 앱 6개 추가 — `ifrs18wp`(IFRS18_wp, 기존 `k1118`과 별개), `startend`(Start and End: JET·LEAD·DSD입력), `qms`, `prerisk`(계약전위험평가조서), `enreport`(영문보고서 초안), `vuln`(취약점 진단). 기존 `toolkit`→"Staff Toolkit", `markettool`→"베타·주가변동성 산출 도구"로 이름·아이콘 교체. 홈 기본 배치에 그룹 `aicase`("AI 활용사례", r4~5·c5~10)로 올림(`ZONES_BUILTIN`·`LAYOUT_DEFAULT`·`finalMemory`). **이미 배치를 저장한 계정은 "배치 초기화"를 눌러야 새 그룹이 보임**(아니면 "모든 App" 서랍에서 끌어다 놓기). 카테고리는 전부 `ai`.
