...
| `expense` | 경비청구 | `extUrl(7000,7005)` → http면 `http://<접속호스트>:7000/`, https면 `https://<접속호스트>:7005/` | **외부 포트 + `auth:'popup'`.** 포털과 같은 MS SSO 를 따라가므로 회의실과 같은 처리 — 포털에 SSO 로 로그인한 상태(`USER.sso`)면 안내 화면 없이 바로 iframe. 창 1280×900 (콘텐츠를 재보지 못해 임시값, 필요하면 `w`·`h` 조정). 2026-09-20 연결 |
