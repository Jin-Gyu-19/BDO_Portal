...
- **AI 활용사례 그룹(2026-09-19)**: 사용자 제공 아이콘 8종(`icons/aicase-*-20260919.webp`, 512px WebP로 변환해 `PORTAL_ASSETS`에 내장)으로 새 앱 6개 추가 — `ifrs18wp`(IFRS18_wp, 기존 `k1118`과 별개), `startend`(Start and End: JET·LEAD·DSD입력), `qms`, `prerisk`(계약전위험평가조서), `enreport`(영문보고서 초안), `vuln`(취약점 진단). 기존 `toolkit`→"Staff Toolkit", `markettool`→"베타·주가변동성 산출 도구"로 이름·아이콘 교체. **홈 기본 배치에는 올리지 않음**(사용자가 직접 배치하기로) — "모든 App" 서랍(카테고리 `ai`)에서 끌어다 놓음.
...
- **다운로드형 앱**(설치 파일·엑셀 매크로 등): **파일만 있고 `url`이 없는 앱은 아이콘을 누르면 바로 내려받음**(`dlDirect()`, `openApp()` 첫머리에서 `triggerDownload()`로 분기, 배지 없음. 2026-09-19). `url`도 있고 파일도 있는 앱만 아이콘 **우측 하단에 작은 다운로드 배지**(`.dlb`)가 붙고, 배지는 다운로드·아이콘 본체는 창. 파일은 NAS `/volume1/sh-pf/docker/nginx-html/portal/downloads/`에 DSM File Station으로 올립니다(nginx 루트가 `portal/`이라 설정 변경 불필요). 편집 모드(`body.editing`)에서는 배지를 숨깁니다.
